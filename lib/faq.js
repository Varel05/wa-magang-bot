import { getFaqContent } from './faqStore.js';

// Sumber awal: Magang.txt
// Ini FALLBACK yang dipakai kalau belum ada FAQ custom tersimpan di Redis
// (mis. pertama kali deploy, sebelum pernah diedit lewat halaman /admin).
export const DEFAULT_FAQ_MAGANG = `
...
function buildSystemPrompt(faqText) {
  return `Anda adalah asisten virtual rekrutmen magang yang ramah, profesional, dan sangat membantu. Tugas Anda adalah menjawab pertanyaan calon peserta magang berdasarkan informasi yang diberikan.
...
/**
 * Ambil system prompt TERKINI — pakai FAQ custom dari Redis kalau sudah pernah diedit
 * lewat halaman /admin, atau fallback ke DEFAULT_FAQ_MAGANG kalau belum.
 * Dipanggil tiap kali mau tanya Gemini, supaya perubahan FAQ langsung berlaku
 * tanpa perlu deploy ulang kode.
 */
export async function getSystemPrompt() {
  const customFaq = await getFaqContent();
  return buildSystemPrompt(customFaq || DEFAULT_FAQ_MAGANG);
}
